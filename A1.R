# stock assessment using ALSCL for operating model 1
library(TMB)
...
for(iter in c(12,29))
{
  ...
  tmb.data=list(
    ...
    len_lower = sim.data$len_mid - 1,
    len_upper = sim.data$len_mid + 1,
  ...
  parameters = list(
    ...
    # random effects
    dev_log_R = rep(0,sim.data$nyear),
    dev_log_F = array(0,c(sim.data$nlen,sim.data$nyear)),
  ...
  parameters.L = list(
    # fixed effects
    log_init_Z = log(0.1),
    log_sigma_log_N0 = log(0.1),
    
    mean_log_R = 1,
    log_sigma_log_R = log(0.01),
    logit_log_R = -20,
    
    mean_log_F = log(0.001),
  ...
  setwd("E:\\simulation study\\assessment models\\ALSCL\\A1")
